Engineering Graphs in Matlab
  Setup
    ...
    engineering plot gui:
      ...
      TripleEng: Triple-specific engineeering plots
    ...
    Decide where you want the Matlab libraries to go.
      ...
      the Triple source code checked out somewhere, so it might make sense
      to leave the Triple-specific engineering plot library where it is.
    ...
    Download Triple engineering plot directory
      cvs -d :ext:forge.abcd.harvard.edu:/cvsroot/arp-triple co -d TripleEng QNX6/eng
        Note that this is part of the Triple source hierarchy, so if you already have
        that checked out, cvs update -d should bring it in.
    
    Add nort, nort/ne and TripleEng directories to your Matlab path
    ...
    Create Triple_Data_Dir.m (probably in your TripleEng directory):
      function path = Triple_Data_Dir
      % path = Triple_Data_Dir;
      path = 'C:\Data\Triple';

  Setup for a specific run:
    Extract engineering data on QNX:
      extract raw/flight/120504.2 tripleengext
    Copy extract .csv files into Triple_Data_Dir
      I have a script called 'getrun' that I place in the Triple_Data_Dir:
